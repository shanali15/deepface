...
def analysis(db_path="data/", model_name='OpenFace', distance_metric= 'cosine', enable_face_analysis = False
				, source = 'rtsp://admin:Epagingadmin@10.194.2.141/cam/realmonitor?channel=1&subtype=1', time_threshold = 1.8, frame_threshold = 1):
	...
	freezed_frame = 0
	tic = time.time()
	cap = cv2.VideoCapture('rtsp://admin:Epagingadmin@10.194.2.141/cam/realmonitor?channel=1&subtype=1') #webcam
	while(True):
		ret, img = cap.read()
		raw_img = img.copy()
						...
						if custom_face.shape[1:3] == input_shape:
							if df.shape[0] > 0: #if there are images to verify, apply face recognition
								...
								if best_distance <= threshold:
									print("Entry into Database Block Image validated")
